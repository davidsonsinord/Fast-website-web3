  ...
  <v-layout row wrap align-center justify-center align-content-space-around>
    ...
    <v-flex v-for="(item, i) in most" :key="i" class="card mobile">
      ...
      <v-card-title class="card-article">
        <div>
          <span class="headline near-black-dav title_article">{{item.title}}</span>
          <v-spacer></v-spacer>
          <div class="w-350">
            <span class="left">{{item.author}}</span>  <span class="right">{{item.date | formatDate}}</span>
          </div>
          <div><v-btn flat class="turquoise-dav pa-0 ma-0 see_more" :to="'/posts/' + item.title">Voir plus <v-icon class="icon">trending_flat</v-icon></v-btn></div>
        </div>
      </v-card-title>
    </v-flex>
    ...
    <v-hover>
      ...
      <v-card-title class="card-article">
        <div>
          ...
          <div>
            <v-btn flat class="turquoise-dav pa-0 ma-0 see_more" :to="'/posts/' + item.title">Voir plus<v-icon class="icon">trending_flat</v-icon></v-btn>
          </div>
        </div>
      </v-card-title>
    </v-flex>
