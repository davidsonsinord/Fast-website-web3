      ...
      <v-container id="id_container" grid-list-xl>
        ...
        >
          <v-flex xs12 sm4>
            <div class="text-xs-center">
              <h2 class="headline ATCArquette-Medium turquoise-dav">
                FORMATIONS & AUDITS & SPEAKER
              </h2>
              <span class="subheading ATCArquette-Light near-black-dav"
                >Un savoir-faire diversifié</span
              >
            </div>
          </v-flex>
        </v-layout>
        ...
        <v-expansion-panel class="mobile">
          <v-expansion-panel-content
            :class="{ 'color-mobile-formation': index % 2 === 0 }"
            v-for="(domain, index) in domains"
            :key="index"
          >
            <template v-slot:header>
              <div class="headline turquoise-dav ATCArquette-Medium">
                {{ domain.title.substring(2, domain.title.length) }}
              </div>
            </template>
            <v-card>
              <v-card-text
                class="text-column-catalog"
                :class="{ 'color-mobile-formation': index % 2 === 0 }"
              >
                <form>
                  <ul>
                    <li
                      v-for="(subdomain, index) in domain.subdomains"
                      :key="index"
                    >
                      <div class="ATCArquette-Bold pt-4 subdomain">
                        {{ subdomain.title }}
                      </div>
                      <ul>
                        <li
          ...
          >
            <v-card class="elevation-0 transparent">
              ...
              <v-card-title
                v-else
                primary-title
                class="layout justify-center ma-0 pa-0"
              >
                <div
                      ...
                      <ul>
                        ...
                        >
                          ...
                          <router-link
                            ...
                            :to="'/formation/' + skill.id"
                            class="black--text plus_btn"
                            >[+]</router-link
                          >
                        </li>
                      </ul>
...
.box {
  -webkit-box-shadow: 0 0 0 0 rgba(0, 0, 0, 0.1), 0 0 0 0 rgba(0, 0, 0, 0.1),
    0 0 15px 2px rgba(0, 0, 0, 0.1) !important;
